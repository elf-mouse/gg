...
(function($) {
    ...
    // fix it
    var elemId, init, sWidth, tWidth, sHeight, tHeight, sTop, sLeft, counter, gapx, tgapx, gapy, tgapy, i, j, effA, eff, delay, from, to2;
    $.fn.fsslider = function(options) {
        init = function(el) {
            ...
            squarePos[elemId] = 0;
            reverse[elemId] = 1;
            params[elemId] = $.extend({},
            $.fn.fsslider.defaults, options);
        ...
        $.transitionCall = function(elemId) {
            ...
            interval[elemId] = setInterval(function() {
                $.transition(elemId);
            },
            delay);
        };
        $.transition = function(elemId, direction) {
            $.effect(elemId);
            squarePos[elemId] = 0;
            appInterval[elemId] = setInterval(function() {
                $.appereance(elemId, order[elemId][squarePos[elemId]]);
            },
            params[elemId].sDelay);
        ...
        $.appereance = function(elemId, sid) {
            ...
            $('#fss-' + elemId + sid).css({
                opacity: 0,
                'background-image': 'url(' + images[elemId][imagePos[elemId]] + ')'
            });
            $('#fss-' + elemId + sid).animate({
                opacity: 1
            },
            300);
            squarePos[elemId]++;
        ...
        $.nav = function(elemId) {
            $('.fss-texture').append("<div id='fss-nav-" + elemId + "' style='position:absolute;z-index:200;bottom:80px;right:80px;'></div>");
